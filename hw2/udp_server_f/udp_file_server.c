...
int main(int argc, char *argv[])
{
	...
	while ((read_cnt = recvfrom(clnt_sd, buf, BUF_SIZE, 0, (struct sockaddr *)&clnt_adr, &clnt_adr_sz)) != 0)
		fwrite((void *)buf, 1, read_cnt, fp);

	puts("Received file data");
	sendto(clnt_sd, "Thank you", 10, 0, (struct sockaddr *)&clnt_adr, clnt_adr_sz);
	fclose(fp);
	close(serv_sd);
	return 0;
}
